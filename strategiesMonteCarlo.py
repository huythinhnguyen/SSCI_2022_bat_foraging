...
def call_strategy(strategy, distance, l_window, r_window, max_turn, min_move):
    ...
    cache = {'iid': iid, 'left': left_sum, 'right': right_sum, 'move': move_rate, 'turn': turn_rate}

    return move_rate, turn_rate, cache


def call_strategy_MC(strategy, distance, l_window, r_window, max_turn, min_move, gamma, delta, wheelbase, freq_call=50):
    if len(gamma)!=2:
        raise ValueError('gamma has to have the shape of (2,)')
    else:
        gamma_l = gamma[0]
        gamma_r = gamma[1]
    if len(delta)!=2:
        raise ValueError('delta has to have the shape of (2,)')
    else:
        delta_l = delta[0]
        delta_r = delta[1]

    move_est, turn_est, cache = call_strategy(strategy, distance, l_window, r_window, max_turn, min_move)
    V_est = move_est * freq_call
    omega_est = np.radians(turn_est) * freq_call

    Vl_est = V_est - (wheelbase/2) * omega_est
    Vr_est = V_est + (wheelbase/2) * omega_est

    V = 0.5 * (gamma_r*delta_r*Vr_est + gamma_l*delta_l*Vl_est)
    omega = (1/wheelbase) * (gamma_r*delta_r*Vr_est - gamma_l*delta_l*Vl_est)

    move = V/freq_call
    turn = np.degrees(omega)/freq_call

    return move, turn, cache


class LocomotionMC:
    def __init__(self, gamma, delta, wheelbase, freq_call=50, strategies=None, glomax = 5.73, window_size = 10):
        if len(gamma)!=2:
            raise ValueError('gamma has to have the shape of (2,)')
        else:
            self.gamma_l = gamma[0]
            self.gamma_r = gamma[1]

        if len(delta)!=2:
            raise ValueError('delta has to have the shape of (2,)')
        else:
            self.delta_l = delta[0]
            self.delta_r = delta[1]
    ...
    def update_MC_para(self, gamma, delta):
        if len(gamma)!=2:
            raise ValueError('gamma has to have the shape of (2,)')
        else:
            self.gamma_l = gamma[0]
    ...
    def update_state(self, echo): # echo, distance2hit, move_rate, turn_rate
        ...
        if self.MC_mode:
            self.move_rate, self.turn_rate, self.cache = call_strategy_MC(self.strategy,
                                                                          self.distance2hit,
                                                                          self.l_window, self.r_window, 
                                                                          self.max_turn_rate, 
                                                                          self.min_move_rate, 
                                                                          [self.gamma_l, self.gamma_r], [self.delta_l, self.delta_r], self.wheelbase)
        else:
